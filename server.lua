                    ...
                    if xPlayer.identifier == result[i].owner then
                        ...
                        for m = 1, #vehicleprices, 1 do
                            if vehicleprices[m].model == model then
                                TriggerEvent('esx_billing:sendBill',  xPlayers[j], 'society_cardealer', 'Araç Vergisi: '..result[i].plate, ((vehicleprices[m].price * yuzde) / 100) , 1)
                                break
                            end
                        end
                    end
...
TriggerEvent("cron", 24, 0, Vergi)
TriggerEvent("cron:runAt", 18, 6, Vergi)